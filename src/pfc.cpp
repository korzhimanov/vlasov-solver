...
PFC::PFC(int particle_type, pyinput *in, Mesh *m, double *n0, double *p0, int *err)
{
...
void PFC::MakeStep(double *ez, double *ax, double *ay)
{
    ...
    {
    #pragma omp for
    for (i = 0; i < mesh->MAX_Z; i++) if (conc[i] > 1e-14)
    {
        ii = i*MAX_P;
        ...
        for (j = 0; j < MAX_P; j++)
        {
            if (*(f1+ii+j) > 1.e-14)
            {
                ...
                {
                    if (j != MAX_P - 1)
                        fold += CorrectedRightSlope(f1+ii+j, f1+ii+j+1) * mymath::onesixth * (1.-flux_part) * (2.-flux_part);
                    if (j != 0)
                        fold += CorrectedLeftSlope (f1+ii+j, f1+ii+j-1) * mymath::onesixth * (1.-flux_part) * (1.+flux_part);
                ...
                {
                    if (j != MAX_P - 1)
                        fold -= CorrectedRightSlope(f1+ii+j, f1+ii+j+1) * mymath::onesixth * (1.-flux_part) * (1.+flux_part);
                    if (j != 0)
                        fold -= CorrectedLeftSlope (f1+ii+j, f1+ii+j-1) * mymath::onesixth * (1.+flux_part) * (2.+flux_part);
    ...
    for (i = 0; i < mesh->MAX_Z; i++) if (conc[i] > 1e-14)
    {
        ...
        for (j = 0; j < MAX_P; j++)
        {
            if (*(f2+ii+j) > 1.e-14)
            {
                ...
                {
                    if (i != mesh->MAX_Z - 1)
                        fold += CorrectedRightSlope(f2+ii+j, f2+ii+j+MAX_P) * mymath::onesixth * (1.-flux_part) * (2.-flux_part);
                    if (i != 0)
                        fold += CorrectedLeftSlope (f2+ii+j, f2+ii+j-MAX_P) * mymath::onesixth * (1.-flux_part) * (1.+flux_part);
                ...
                {
                    if (i != mesh->MAX_Z - 1)
                        fold -= CorrectedRightSlope(f2+ii+j, f2+ii+j+MAX_P) * mymath::onesixth * (1.-flux_part) * (1.+flux_part);
                    if (i != 0)
                        fold -= CorrectedLeftSlope (f2+ii+j, f2+ii+j-MAX_P) * mymath::onesixth * (1.+flux_part) * (2.+flux_part);
...
{
    double slope = *rightvalue - *centralvalue,
           slope_c;
    if (slope < -1e-14)
    {
...
{
    double slope = *centralvalue - *leftvalue,
           slope_c;
    if (slope > 1e-14)
    {
        slope_c = 2.*(maxf-*centralvalue);
