...
class Plasmas
{
    public:
        int species_number;
        double critical_concentration, // critical concentration in a boosted frame (useful for simulation of oblique irradiation)
...
inline void Plasmas::CalcLongFields()
{
    ...
    for (int i = 1; i < mesh->MAX_Z; i++)
        ez[i] += ez[i-1] + 0.5*critical_concentration*mesh->dz*(fixed_ions_conc[i]+fixed_ions_conc[i-1]);
    ez[mesh->MAX_Z] += 0.5*critical_concentration*mesh->dz*fixed_ions_conc[mesh->MAX_Z-1];
}
